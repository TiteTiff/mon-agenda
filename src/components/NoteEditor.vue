...
const content = ref(props.note?.text || '')
  ...
  <div class="bg-deep text-cream p-4 lg:mx-48 pb-16 min-h-screen">
    <div class="flex flex-row justify-between items-center">
      ...
      <div class="flex justify-between gap-2 mb-4 flex-1 mr-4">
        ...
        <input v-model="title" type="text" placeholder="Nouvelle note" class="placeholder-cream w-full text-xl text-cream font-sans font-medium bg-transparent border-0 border-b-2 border-transparent focus:border-accent outline-none"/>
      </div>
    ...
    <div class="bg-surface border border-border rounded-lg  p-4">
      ...
      <footer class="text-sm text-muted border-t border-border pt-2 mt-2"> Créé le {{ props.note?.updated_at || new Date().toLocaleDateString('fr-FR') }}</footer>
    </div>
