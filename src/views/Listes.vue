...
<template>
  <div class="bg-deep flex flex-col p-4 pb-16">
    ...
    <div class="flex items-center justify-between min-h-15 mb-md">
      ...
      <button @click="showForm = true" class="bg-accent text-cream px-md py-md rounded-md text-md flex items-center gap-sm" >
    ...
    <div class="bg-surface border border-border rounded-lg p-sm mt-sm flex flex-col" v-if="showForm">
      <form @submit.prevent="addList" class="flex flex-col gap-sm p-md">
        <input autofocus v-model="newList" type="text" required="required" placeholder="Nom de la liste" class="bg-raised text-cream py-md px-md border border-border rounded-lg focus:border-accent outline-none" />
        <div class="flex gap-sm">
          <input type="submit" value="Créer" class="bg-accent text-cream border-border rounded-lg w-full py-md" />
          <button @click="showForm = false" class="text-cream bg-surface border-border rounded-lg px-lg py-md"> Annuler</button>
        </div>
      </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mt-md">
      ...
      <button class="bg-surface border border-border rounded-lg hover:border-accent transition-colors p-4 lg:p-10 flex flex-col group" v-for="list in checklists" :key="list.id" >
        <div>
          ...
          <div class="w-full rounded-full bg-raised">
            <div class="bg-success h-2 rounded-full" :style="{ width: (list.items_total == 0 ? 0 : (list.items_checked / list.items_total * 100)) + '%' }">
            </div>
          </div>
  ...
  </div> <!--<div v-for="item in list.items" :key="item.id">
          <label>
